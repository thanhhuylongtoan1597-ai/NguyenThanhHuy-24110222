def expectimax(board, depth, remaining_depth, is_maximizing, ai_player, nodes_evaluated):
    nodes_evaluated[0] += 1

    winner, _ = board.check_winner()
    human_player = 'O' if ai_player == 'X' else 'X'

    if winner == ai_player:
        return 10 - depth
    elif winner == human_player:
        return -10 + depth
    elif winner == 'Draw':
        return 0

    if remaining_depth == 0:
        return board.evaluate_heuristic(ai_player)

    if is_maximizing:
        val = -float('inf')
        for r, c in board.get_empty_cells():
            board.make_move(r, c, ai_player)
            val = max(val, expectimax(board, depth + 1, remaining_depth - 1, False, ai_player, nodes_evaluated))
            board.undo_move(r, c)
        return val
    else:
        val = 0.0
        empty_cells = board.get_empty_cells()
        if not empty_cells:
            return 0.0
        probability = 1.0 / len(empty_cells)
        for r, c in empty_cells:
            board.make_move(r, c, human_player)
            val += probability * expectimax(board, depth + 1, remaining_depth - 1, True, ai_player, nodes_evaluated)
            board.undo_move(r, c)
        return val
