...
│       ├── sensorless_search.py # Tìm kiếm không cảm biến (Sensorless Search)
│       ├── and_or_search.py  # Tìm kiếm quan sát một phần / cây AND-OR
│       └── partial_observation_search.py # Tìm kiếm quan sát một phần (Partial Observation Search)
│
  ...
  def UCS(problem):
      ...
      while not frontier.is_empty():
          ...
          for action in problem.actions(node.state):
              ...
              if child.state not in explored and child.state not in frontier:
                  frontier.push(child)
              elif child.state in frontier and child.path_cost < frontier[child.state].path_cost:
...
* **Mã giả (Pseudo-code):**
  ...
  def IDS(problem):
      for depth in range(0, infinity):
          result = Depth_Limited_Search(Node(state=problem.initial_state), problem, depth)
  ...
  def A_Star(problem):
      ...
      while not frontier.is_empty():
          ...
          for action in problem.actions(node.state):
              ...
              if child.state not in explored and child.state not in frontier:
                  frontier.push(child)
              elif child.state in frontier and child.f < frontier[child.state].f:
                  frontier.update(child)
      return None
  ```
...
* **Mã giả (Pseudo-code):**
  ...
  def Simulated_Annealing(problem, schedule):
      ...
      for t in range(1, infinity):
          ...
          # Trường hợp Cực tiểu hóa chi phí/Heuristic (Minimization - tương tự code thực tế của bài toán)
          delta = heuristic(next_state) - heuristic(current)
          if delta < 0:
              current = next_state
          else:
              # Chấp nhận trạng thái xấu hơn (chi phí cao hơn) với xác suất e^(-delta / T)
              if random_probability() < e**(-delta / T):
                  current = next_state
  ```
...
<img width="1090" height="888" alt="Đang ghi 2026-06-29 230212" src="https://github.com/user-attachments/assets/fff8bf53-6ae9-4787-aa9b-5b14db0891c1" />
---
...
* **Mã giả (AC-3 Algorithm):**
  ...
  def Revise(csp, Xi, Xj):
      ...
      for x in csp.domains[Xi]:
          if not any(satisfies_constraint(x, y) for y in csp.domains[Xj]):
...
<img width="400" height="241" alt="Đang ghi 2026-06-29 231434" src="https://github.com/user-attachments/assets/a0fb9204-227a-45a9-a847-35d569afc23e" />
...
<img width="1180" height="848" alt="20260629-1618-35 7829475" src="https://github.com/user-attachments/assets/923424c0-fe80-4915-8edf-5a7c9aba8599" />
